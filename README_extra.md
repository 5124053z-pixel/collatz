...
So the family of questions here — parameterized by (L, x) instead of just "distance 1" — appears to be a genuine generalization of the classical ~50% phenomenon. ~~I could not find an existing rigorous proof or heuristic derivation of even the base L=1 case in the literature searched.~~ **Correction (literature survey, see §6): this is wrong — a partial rigorous treatment of the base L=1 case exists and dates to L.E. Garner, "On Heights in the Collatz 3n+1 Problem," *Discrete Math.* 55 (1985), 57–64, with the general "clustering" phenomenon studied continuously since (G.-G. Gao, *Discrete Math.* 112 (1993); P. Andaloro, *Fibonacci Quart.* 38 (2000); M.D. LaDue, arXiv:1709.02979 (2017)).** The search that produced this README's original claim was insufficient; see §6 for the full correction and what does still appear to be new. (Terras's theorem characterizes the ordinary stopping time — first drop below the starting value — by finite congruence classes, but total stopping time is not known to reduce to finite congruence data, which is presumably why this is hard — that part of the original claim stands.)

### 5b. A proved partial result for the L=1 base case

**Correction (added after a literature survey, see §6): this is not a new result.** The "theorem" below is n≡4 (mod 8) ⟹ steps(n)=steps(n+1), which is exactly Corollary 5.1 of Mark D. LaDue, "Clusters of Integers with Equal Total Stopping Times in the 3x+1 Problem" (arXiv:1709.02979 / *Fibonacci Quarterly*), stated there as σ∞(8i+4)=σ∞(8i+5) — and LaDue in turn derives it as a special case of L.E. Garner, "On Heights in the Collatz 3n+1 Problem," *Discrete Math.* **55** (1985), 57–64. The general *methodology* here (enumerating residue classes mod 2^k that provably force n,n+1 to merge) also parallels, rather than originates, technique used in LaDue 2017 and in Mike Winkler, "Deterministic Structures in the Stopping Time Dynamics of the 3x+1 Problem" (arXiv:1709.03385). This section is kept as originally written (independently re-derived, not copied) but should be read as **independent rediscovery**, not a first proof — see §6 for the full citation trail and what in this README does appear to be new.
...
Theorem (independently re-derived; see correction above — first proved by Garner 1985). For all n ≡ 4 (mod 8) with n ≥ 12, write n = 8j+4. Then:
...
**Note (literature survey, see §6): a related but distinct result already existed.** G.-G. Gao, "On Consecutive Numbers of the Same Height in the Collatz Problem," *Discrete Math.* 112 (1993), 261–267, computationally found a run of 35,654 *consecutive* integers all sharing the same total stopping time, and conjectured (via heuristic) that such runs grow without bound. That is a statement about longest-run-length, not directly about the pairwise probability P(steps(n)=steps(n+1)) at scale n examined here — no paper stating this specific pairwise-limit-to-1 claim was found in the survey — but the two are clearly related manifestations of the same underlying phenomenon, and this section should be read as a probabilistic restatement/extension of Gao's observation rather than a fully independent discovery.
...
**This section was rewritten after a literature survey (2026-07-26)** that fact-checked the citations previously here (which came from an earlier AI-assisted pass and had not been independently verified) and searched specifically for prior work on this README's central claims. The corrected picture:

**What was miscited before.** The "a number's first m Collatz steps are determined by n mod 2^m" fact (used in §3's proof) was previously attributed to Bernstein & Lagarias 1996. That paper is real (D.J. Bernstein & J.C. Lagarias, "The 3x+1 Conjugacy Map," *Canadian J. Math.* 48 (1996), 1154–1169) but its actual content is the cycle structure of the induced 2-adic conjugacy map on Z/2ⁿZ (proving order 2ⁿ⁻⁴ for n≥6) — it does not state the mod-2^m parity-vector fact as its contribution. That fact is older and more elementary, properly attributed to R. Terras, "A Stopping Time Problem on the Positive Integers," *Acta Arithmetica* 30 (1976), 241–252 (with correcting addendum, *Acta Arithmetica* 35 (1978), 101–102), or to Lagarias's 1985 survey ("The 3x+1 problem and its generalizations," *Amer. Math. Monthly*). Similarly, "Collatz cyclic numbers" was previously bundled with the Bernstein-Lagarias citation as if from the same source; it isn't — it traces to later work such as arXiv:2007.06979.

**The 2-adic framing itself still holds.** As k → ∞, N_k converges 2-adically to the rational 2-adic integer α = -x/(2^L - 1), placing the §1-4 construction inside the well-studied 2-adic extension of the Collatz map (Bernstein & Lagarias 1996, correctly cited for *this* claim about the conjugacy map's structure).

**§5b's "theorem" is not new — important correction.** The survey found that n≡4 (mod 8) ⟹ steps(n)=steps(n+1) is a known result: it is Corollary 5.1 of M.D. LaDue, "Clusters of Integers with Equal Total Stopping Times in the 3x+1 Problem" (arXiv:1709.02979, *Fibonacci Quarterly*), which LaDue derives as a special case of L.E. Garner, "On Heights in the Collatz 3n+1 Problem," *Discrete Math.* 55 (1985), 57–64 (Theorem 1). The broader "clustering" phenomenon — runs of consecutive integers sharing a total stopping time — has been studied continuously since: G.-G. Gao, "On Consecutive Numbers of the Same Height in the Collatz Problem," *Discrete Math.* 112 (1993), 261–267 (found a 35,654-integer run, conjectured unbounded growth); P. Andaloro, "On Total Stopping Times Under 3X+1 Iteration," *Fibonacci Quart.* 38 (2000), 73–78. The general merging-class *methodology* of §5b/§5c also parallels (not originates) technique used in LaDue 2017 and in M. Winkler, "Deterministic Structures in the Stopping Time Dynamics of the 3x+1 Problem" (arXiv:1709.03385), which builds an analogous recursive tree of congruence classes. §5b, §5c, and the earlier framing of this section should be read as **independent rediscovery and extension** of a ~40-year-old line of work, not a first proof — see the correction notes now added at §5a and §5b.

**§5d's rising-agreement-rate claim** (P(steps(n)=steps(n+1)) → 1, not a stable ~50%) is related to but distinct from Gao 1993's cluster-length heuristic (see the note at the end of §5d); no paper stating this specific pairwise-probability claim was found.

**The Addendum's coupling/coalescence framing.** The underlying fact — that Collatz trajectories of related integers can literally merge to a common value before reaching 1 — is old, essentially folklore in the field (it's the standard justification for memoization in Collatz-search code, and exactly the mechanism Garner/Gao/Andaloro/LaDue formalize for specific classes). One directly on-topic paper uses "coalescence" terminology for a *different* pairing: R. Burson, "On the Distribution of the First Point of Coalescence for some Collatz Trajectories" (arXiv:2005.09456, 2020, later withdrawn/merged into another paper), studying coalescence of n and 3n+2. The Addendum's specific contribution — a formal τ_couple(n) definition, the quantile-scaling analysis showing a two-component (bits-independent fast bulk + linearly-growing heavy tail) mixture, and the exhaustive zero-exception verification — was not found duplicated in the literature searched.

**General background, not directly on point:** T. Tao, "Almost all Collatz orbits attain almost bounded values," arXiv:1909.03562 (*Forum of Math. Pi*, 2019); C.J. Everett, "Iteration of the number theoretic function," *Adv. Math.* 25 (1977), 42–45; I. Korec's density bound. These are general Collatz-orbit-statistics results, useful context but not prior art for the specific claims here. Cycle-nonexistence background: S. Eliahou, "The 3x+1 problem: new lower bounds on nontrivial cycle lengths," *Discrete Math.* 118 (1993), 45–56; J.L. Simons & B.M.M. de Weger, "Theoretical and computational bounds for m-cycles of the 3n+1-problem," *Acta Arithmetica* 117 (2005), 51–70; C. Hercher, "There are no Collatz m-Cycles with m≤91," *J. Integer Seq.* 26 (2023), Art. 23.3.5 (arXiv:2201.00406) — supersedes Simons–de Weger's bound.

**What does appear to be new, per this search:** the §1-4 repeated-block construction itself (no prior work found on repunit-like self-similar bit-block constructions and their effect on total stopping time), and §13's finding that rescaling the *input* by 3^p (keeping the ordinary 3n+1 map fixed) produces a clean diff=-p effect — the one adjacent-sounding paper found, N. Boulkaboul, "3n+3^k: New Perspective on Collatz Conjecture" (arXiv:2212.00073), studies a different construction (changing the map's additive constant, not rescaling the input). As always, "not found in the literature searched" is evidence of possible novelty, not proof of it — corrections and pointers to prior work remain very welcome.
