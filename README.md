...
**This file is a short summary.** The full investigation — every table, the coupling-theory Addendum, the honest record of mistakes made and corrected along the way, and the newest power-of-3 generalization — is in **[README_extra.md](README_extra.md)**, organized into 20 sections + Addendum. This file exists so a new reader isn't faced with 500+ lines up front.
...
| ~~7~~ | ~~Agreement of `steps(n)=steps(n+1)` is fully explained by literal early trajectory coalescence — zero exceptions found anywhere tested~~ | **RETRACTED — tautology.** Both trajectories are at value 2 at step `s−1` (2 is the only positive preimage of 1), so a strict early merge always exists; the 100%/0-exception result was forced and the claimed equivalence is a two-line theorem. Third instance of the trap Addendum §1 documents. What survives: the *distribution* of `tau_couple` (merges occur near `0.12·s`, not forced) — see #8 ([§20](README_extra.md#20-audit-addendum-2s-100-early-merge-zero-exceptions-is-a-tautology-third-instance-of-the-same-trap)) |
...
- ~~No proof that zero-exception early-merging holds for *all* n (only verified exhaustively/by sampling to large but finite scales).~~ **Dissolved by §20:** it holds for all n, trivially, and says nothing. The real version of this question — whether agreement is ever "coincidental" rather than mechanism-driven — was never actually tested by result #7 and remains open.
...
README_extra.md              full write-up (20 sections + Addendum)
LICENSE                      MIT
...
§10-20 follow-ups (this session's independent re-verification + new findings):
